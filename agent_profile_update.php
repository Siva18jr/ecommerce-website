...
if(isset($_POST['update_profile'])){
    ...
    $name = $_POST['name'];
    $email = $_POST['email'];
    mysqli_query($conn, "UPDATE `users` SET name = '$name', email='$email' WHERE id = '$agent_id'") or die('query failed');
    ...
    if(!empty($update_pw) AND !empty($new_pw) AND !empty($c_pw)){
        ...
        if($update_pw != $old_pw){
            ...
            mysqli_query($conn, "UPDATE `users` SET password='$new_pw' WHERE id = '$agent_id'") or die('query failed');
...
<section class="update-profile">
   <form action="" method="POST">
      ...
      $res = mysqli_query($conn, "SELECT * FROM `users` WHERE id='$agent_id'") or die('query failed');
