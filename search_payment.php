...
if(isset($_GET['delete'])){
   ...
   mysqli_query($conn, "DELETE FROM `agent_payment` WHERE id = '$delete_id'") or die('query failed');
...
<h1 class="title">all agent payment </h1><br>
   <table>
      <thead>
         <th>id</th>
         <th>user id</th>
      ...
      <tbody>
         <?php
            ...
            {
              echo "<center><h1>Items added between ".$new_sdate." and ".$new_edate."</h1></center>";
            }
            $result=mysqli_query($conn, "Select id,user_id,name,final_price,status,placed_on From `agent_payment` Where placed_on Between '$new_sdate' and '$new_edate' or placed_on='$new_sdate' order by placed_on");
         ...
         <tr>
            ...
            <td>
            <form action="admin_agent_payment.php" method="post">
            ...
            <td id="remove">
               <a id="remove" href="admin_agent_payment.php?delete=<?php echo $row['id']; ?>" class="btnn" onclick="return confirm('remove this order?');"> remove</a>
            </td>
         </tr>
